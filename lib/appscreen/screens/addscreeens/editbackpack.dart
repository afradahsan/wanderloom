...
class EditBackpack extends StatefulWidget {
  const EditBackpack({required this.tripId, required this.backpackId, super.key});

  final String tripId;
  final String backpackId;

  @override
  State<EditBackpack> createState() => _EditBackpackState();
}
...
class _EditBackpackState extends State<EditBackpack> {
  ...
  Future onAdd() async{
    if(itemController.toString().isNotEmpty && _choiceChipsList[_selectedIndex!].label.toString().isNotEmpty){
      ...
      await DatabaseService().updateBackpack(itemName, itemCategory, userId!, widget.tripId, widget.backpackId);
