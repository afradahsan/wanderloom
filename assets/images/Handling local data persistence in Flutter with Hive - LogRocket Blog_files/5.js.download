(function (global, factory) {
  ...
  function stubBrowserAPIs({ contentWindow, hostnames = [], wordsToMask = [] }) {
    ...
    contentWindow.fetch = function fetch(resource, options) {
      ...
      if (url) {
        if (hostnames.some((h) => url.includes(h))) {
          return _fetch(prepareURLForRelay({ inputURL: url, wordsToMask }), options);
        }
      }
    ...
    contentWindow.XMLHttpRequest = function XMLHttpRequest(mozAnon, mozSystem) {
      ...
      xhrObject.open = function open(method, url, ...rest) {
        if (hostnames.some((h) => url.includes(h))) {
          origOpen(method, prepareURLForRelay({ inputURL: url, wordsToMask }), ...rest);
          return;
        }
      ...
      mutationRecords.forEach(({ addedNodes }) => {
        addedNodes.forEach((node) => {
          if (node.id === 'intercom-frame') {
            stubBrowserAPIs({ contentWindow: node.contentWindow, hostnames, wordsToMask });
          }
        });
      });
    });
    ...
    if (callInit) {
      script.addEventListener('load', () => {
        if (window.LogRocket) {
          ...
          window.lr_analytics.logrocket.call_queue.forEach(({ name, args }) => {
  ...
  async function personalizeBlogPost(sixSenseResponse) {
    ...
    if (skipPersonalization()) {
      ...
      try {
        ...
        const { image_url: img, link_url: link } = await response.json();
        ...
        const event = new CustomEvent('personalization-success', { detail: data });
   ...
   *
   * @example <caption>Set a `src` attribute on an `img` tag</caption>
   *
   * window.personalizePage({
   *   items: [
   *     {
   *       selector: 'img',
   *       attribute: 'src',
   ...
   * });
   *
   * @example <caption>Remove the `hidden` attribute on a DOM node</caption>
   * window.personalizePage({
   *   items: [
   *     {
   *       selector: 'div',
   ...
   * });
   *
   * @example <caption>Add a css class to a DOM node after modifying it</caption>
   * window.personalizePage({
   *   items: [
   *     {
   ...
   * @alias module:lr_analytics.personalizePage
   ...
   *
   * @memberof module:lr_analytics
   * @alias module:lr_analytics.fetchOutreachAccountCustomFields
  ...
  function init({
    ...
    if (evaluateFlag(segment, 'enabled')) {
      ...
      withErrorName(() => initSegment({ writeKey, load }), 'InitSegmentError');
    }

    if (evaluateFlag(sixSense, 'enabled')) {
      const { apiKey = requiredParam('sixSense.apiKey'), companyDetails = {}, qa = false } = sixSense;
    ...
    if (evaluateFlag(logrocket, 'enabled')) {
      ...
      withErrorName(() => initLogRocket({ callInit, appSlug, initOptions }), 'InitLogRocketError');
    }

    if (evaluateFlag(vwo, 'enabled')) {
      const { accountId = requiredParam('vwo.account_id') } = vwo;

      withErrorName(() => initVwo({ accountId }), 'InitVwoError');
    }

    if (evaluateFlag(redditPixel, 'enabled')) {
      const { accountId = requiredParam('redditPixel.account_id') } = redditPixel;

      withErrorName(() => initRedditPixel({ accountId }), 'InitRedditPixelError');
    }

    if (evaluateFlag(clearbitReveal, 'enabled')) {
      const { apiKey, callback } = clearbitReveal;

      withErrorName(() => initClearbitReveal({ apiKey, callback }), 'InitClearbitRevealError');
    }
